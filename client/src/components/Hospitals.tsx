...
const Hospitals: React.FC<HospitalsProps> = ({ hospitals, searchQuery }) => {
    ...
    const hospitalBody = useMemo(() => {
        ...
        <div key={index} className="flex flex-col items-start gap-4  cursor-pointer shadow-lg hover:shadow-light-black">
                ...
                <div className="flex flex-col items-start gap-3 pl-2">
                    <div className="flex flex-col items-start gap-1">
                        <div className="text-light-black font-satoshi text-xl non-italic leading-7 font-medium">{hospital.name}</div>
                        <div className="text-ash font-satoshi text-base leading-6 font-normal">{hospital.location}, { hospital.state}</div>
                    </div>
                    <div>&#11088; <span className="text-light-black font-satoshi text-base leading-6 non-italic font-medium">4.5</span></div>
                </div>
          </div>  
        ))
    }, [filteredHospitals])
    return (
        <div className="inline-flex flex-col items-center w-[100%] pt-20 gap-[32px]">
            ...
            <div className="grid grid-cols-4 grid-rows-2 gap-[16px] pr-6 pl-6">{ hospitalBody }</div>
            <div>
                <Button
